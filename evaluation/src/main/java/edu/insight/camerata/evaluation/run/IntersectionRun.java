...
public class IntersectionRun {
	...
	public static void main(String[] args) {
		...
		System.out.println(questions);
		for(Question question : questions) {
			if(question.number.equals("186")){
				System.out.println("debug");
			}
			answerXml.append("<question dieisions=" + "\"" + question.divisions + "\"" + " music_file=\"" + question.musicXmlFile + "\" " + 
					"number=\"" + question.number + "\">\n");
			answerXml.append("<text>" + question.question + "</text>\n");
			answerXml.append("<answer>\n");
			Music music = new Music();			
			...
			for(Measure measure : answerMeasures) {
				if(notes.size()>1){ //if question contains more than 1 notes, then just giving the first intersection as answer: need to guess why
